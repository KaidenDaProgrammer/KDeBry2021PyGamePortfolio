...
class CookieDisplayObj:
    ...
    def draw(self, score, user_cps):
        ...
        small_font = pygame.font.SysFont('arial', 15, True)
...
class Building:
    ...
    def draw(self, solid=True):
        ...
        store_cost_font = pygame.font.SysFont('Font/ChelseaMarket-Regular.ttf', 17, True)
        store_quantity_font = pygame.font.SysFont('Font/ChelseaMarket-Regular.ttf', 36, True)
    ...
    def drawDisplayBox(self):
        building_font = pygame.font.SysFont('Font/ChelseaMarket-Regular.ttf', 20, True)
        building_title = building_font.render('{}'.format(self.name), True, WHITE)

        description_font = pygame.font.SysFont('Font/ChelseaMarket-Regular.ttf', 12, True)
        ...
        quantity = description_font.render(
            'You have {} {}s producing {:.1f} cookies per second'.format(self.quantity, self.name,
                                                                         self.cps * self.quantity), True, WHITE)
        created = description_font.render(
            '{}s have created {} cookies so far'.format(self.name, math.floor(self.created)), True, WHITE)
    ...
    def addUpgrade(self):
        ...
        if self.name == 'Cursor':
            if self.quantity == I:
                list_of_upgrades.append(
                    Upgrade('Reinforced Index Finger I', cost=self.getTotalCost() * 10, upgrade=self.name))
                list_of_upgrades.append(Upgrade('Double Click', cost=1000, upgrade='Mouse'))
            elif self.quantity == II:
                list_of_upgrades.append(
                    Upgrade('Arthritis prevention cream', cost=self.getTotalCost() * 10, upgrade=self.name))
            ...
            if self.quantity == I:
                list_of_upgrades.append(
                    Upgrade('Forwards from grandma', cost=self.getTotalCost() * 10, upgrade=self.name))
            elif self.quantity == II:
                list_of_upgrades.append(
                    Upgrade('Steel-plated rolling pins', cost=self.getTotalCost() * 10, upgrade=self.name))
            elif self.quantity == III:
                list_of_upgrades.append(
                    Upgrade('Lubricated dentures', cost=self.getTotalCost() * 10, upgrade=self.name))
        elif self.name == 'Farm':
            if self.quantity == I:
            ...
            if self.quantity == I:
                list_of_upgrades.append(
                    Upgrade('Sturdier conveyor belts', cost=self.getTotalCost() * 10, upgrade=self.name))
            elif self.quantity == II:
...
class Upgrade:
    ...
    def drawDisplayBox(self):
        upgrade_font = pygame.font.SysFont('Font/ChelseaMarket-Regular.ttf', 20, True)
        upgrade_title = upgrade_font.render('{}'.format(self.name), True, WHITE)

        cost_font = pygame.font.SysFont('Font/ChelseaMarket-Regular.ttf', 14, True)
...
grandma = Building('Grandma', 700, store_y + 64 * 1, grandma_img, grandma_icon, base_cost=100,
                   increase_per_purchase=1.15, cps=1)
...
factory = Building('Factory', 700, store_y + 64 * 4, factory_img, factory_icon, base_cost=130000,
                   increase_per_purchase=1.15, cps=260)
bank = Building('Bank', 700, store_y + 64 * 5, bank_img, factory_icon, base_cost=1400000, increase_per_purchase=1.15,
                cps=1400)
temple = Building('Temple', 700, store_y + 64 * 6, temple_img, temple_icon, base_cost=20000000,
                  increase_per_purchase=1.15, cps=7800)
wizard_tower = Building('Wizard Tower', 700, store_y + 64 * 7, wizard_tower_img, wizard_tower_icon, base_cost=330000000,
                        increase_per_purchase=1.15, cps=311080)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.QUIT:
            ...
            if cookie.collidepoint(mouse_pos):
                ...
                list_of_clicked_cookies.append(
                    BackgroundCookie(pygame.mouse.get_pos()[0] - 20, pygame.mouse.get_pos()[1] - 20))
